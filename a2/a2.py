...
def tokenize(doc, keep_internal_punct=False):
    ...
    if keep_internal_punct==False:
        words = re.sub('\\W+', ' ', doc.lower()).split()
    else:
        words = re.sub(' *[\?!][\'"\)\]]* *', ' ', doc.lower()).split()
...
def token_features(tokens, feats):
    ...
    """
    ###TODO
    tokens=["token=" + str(x) for x in tokens]
    for i in tokens:
        if i not in feats:
            feats[i] =1
        else:
...
def token_pair_features(tokens, feats, k=3):
    ...
    a=[]
    b=[]
    for x in range(len(tokens) - k + 1):
        a.append(([tokens[x], tokens[x + 1], tokens[x + 2]]))
    for i in range(len(a)):
        for combo in combinations(a[i], 2):
            b.append('__'.join(combo))
    for s in b:
        if 'token_pair='+s in feats:
            feats['token_pair='+s]+=1
        else:
            feats['token_pair='+s]=1
    pass
...
def lexicon_features(tokens, feats):
    ...
    >>> feats = defaultdict(lambda: 0)
    >>> lexicon_features(np.array(['i', 'LOVE', 'this', 'great', 'boring', 'movie', 'worst', 'horrible']), feats)
    >>> sorted(feats.items())
    [('neg_words', 3), ('pos_words', 2)]
    """
    ###TODO
    neg=[]
    pos=[]
    ...
    feats.setdefault("neg_words",len(neg))
    feats.setdefault("pos_words",len(pos))
    pass
...
def vectorize(tokens_list, feature_fns, min_freq, vocab=None):
    ...
    for i in range(len(tokens_list)):
        a = featurize(tokens_list[i],feature_fns)
        for term in a:
            if term[0] in vob_dict:
                vob_dict[term[0]] = vob_dict[term[0]]+1
            else:
                vob_dict[term[0]] = 1
                
            #if term[0] not in vocabulary:
            #        vocabulary.append(term[0])  
        all_features[i]=a
        
    for key in vob_dict:
        if vob_dict[key] >= min_freq:
            if key not in vob_list:
                vob_list.append(key)
    vob_list = sorted(vob_list)
        
    for term in sorted(vob_dict):
        vocab[term]=count
        count=count+1
        
    for i,doc_features in all_features.items():
...
def eval_all_combinations(docs, labels, punct_vals,
    ...
    for punc in punct_vals:
        ...
        for l in range(len(feature_fns)):
            for comb in combinations(feature_fns, l+1):
                for min_f in min_freqs:
                    clf = LogisticRegression()
                    t_dict = {}
                    t_dict['features'] = comb
                    t_dict['punct'] = punc
                    t_dict['min_freq'] = min_f
                    X, vocab = vectorize(token_array, comb, min_f)
                    t_dict['accuracy'] = cross_validation_accuracy(clf, X, labels, 5)
                    rl.append(t_dict)
    retl = sorted(rl, key=lambda k: k['accuracy'])
    return retl
    pass
...
def top_coefs(clf, label, n, vocab):
    ...
    bot_coef_ind = np.argsort(coef)
    for key,val in sorted(vocab.items()):
        bot_coef_terms.append(key)
    bot_coef = coef[bot_coef_ind]
    
    pos_coef = list(zip(top_coef_terms, top_coef))[:n]
    neg_coef = list(zip(bot_coef_terms, bot_coef))[:n]
...
def print_top_misclassified(test_docs, test_labels, X_test, clf, n):
    ...
    for index, d in enumerate(test_docs):
        if (predict_labels[index] != test_labels[index]):
            errors.append(dict((["probas", probabilities[index][predict_labels[index]]],
                                ["truth", test_labels[index]],
                                ["predicted", predict_labels[index]],
                                ["test_docs", test_docs[index]])))
    errors=sorted(errors, key=lambda x:x['probas'], reverse=True)[:n]
    for i in errors:
        print("truth=", i['truth'], "predicted=", i['predicted'], "proba=", i['probas'])
        print(i['test_docs'])
    pass
