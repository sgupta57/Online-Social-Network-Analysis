...
def print_num_friends(users):
    ...
    u=len(users)
    for i in range(u):
       a=(str(users[i]['screen_name']))
       b=(str(len(users[i]['friends'])))
       c=(a+" "+ b)
       print(c)
...
def friend_overlap(users):
    ...
    u=len(users)
    for s in range(u):
        a=( users[s]['screen_name'])
        for t in range(u):
            b=( users[t]['screen_name'])
...
def followed_by_hillary_and_donald(users, twitter):
    ...
    #print (d)
    y=robust_request(twitter,'users/lookup',{'user_id':d}).json()
    for u in y:
...
def create_graph(users, friend_counts):
    ...
    u=len(users)
    for i in range(u):
       a=(users[i]['screen_name'])
       graph.add_node(a)
    ###for i in range(u):
        ###b=(users[i]['friends'])
       for k in (users[i]['friends']):
            if (friend_counts[k] > 1):
                graph.add_node(k)
                graph.add_edge(k, users[i]['screen_name'])
    return graph
    pass
...
def draw_network(graph, users, filename):
    ...
    for i in range(u):
       b.append(users[i]['screen_name'])
    dic={a: a for a in graph.nodes() if a in b }
    nx.draw_networkx(graph, width = 0.5, font_size = 10, label=dic)
